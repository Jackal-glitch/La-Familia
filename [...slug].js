...
const DEFAULT_SETTINGS = { company: 'La Familia', currency: '$', defaultRate: 20, maxDays: 60, timezone: 'UTC' };
...
const SCHEMA = [
  ...
  `CREATE TABLE IF NOT EXISTS loans (
    ...
    principal REAL NOT NULL CHECK (principal > 0), rate REAL NOT NULL, days INTEGER NOT NULL,
    start_date TEXT NOT NULL, due_date TEXT NOT NULL, interest REAL NOT NULL, total_due REAL NOT NULL,
...
async function snapshot(db, settings, logLimit = 120) {
  ...
  return {
    ...
    loans: ln.map(l => ({
      id: l.id, clientId: l.client_id, principal: l.principal, rate: l.rate, days: Number(l.days), startDate: l.start_date, dueDate: l.due_date,
      interest: l.interest, totalDue: l.total_due, guarantee: l.guarantee, notes: l.notes, payments: pays.get(l.id) || [],
...
const PROXY_HEADERS = ['x-forwarded-for', 'x-real-ip', 'forwarded', 'cf-connecting-ip', 'cf-ray', 'x-vercel-id'];
...
const ipOf = req => String(req.headers['x-vercel-forwarded-for'] || req.headers['x-real-ip'] || req.headers['cf-connecting-ip'] || String(req.headers['x-forwarded-for'] || '').split(',')[0] || (req.socket && req.socket.remoteAddress) || '?').trim();
...
add('POST', '/api/loans', 'admin', async ({ db, cfg, user, body }) => {
  ...
  if (!Number.isInteger(days) || days < 1 || days > s.maxDays) throw new HttpError(400, `La durée doit être comprise entre 1 et ${s.maxDays} jours`);
  const rate = body.rate === '' || body.rate == null ? s.defaultRate : Number(body.rate);
  if (!Number.isFinite(rate) || rate < 0 || rate > 100) throw new HttpError(400, 'Le taux doit être compris entre 0 et 100 %');
  ...
  return mutate(db, async t => {
    ...
    const interest = round2(principal * rate / 100), totalDue = round2(principal + interest), dueDate = addDays(startDate, days);
    await t.run('INSERT INTO loans(id, client_id, principal, rate, days, start_date, due_date, interest, total_due, guarantee, notes, written_off_date, created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,NULL,?)',
      [loanId, clientId, principal, rate, days, startDate, dueDate, interest, totalDue, str(body.guarantee, 200), str(body.notes, 1000), nowIso()]);
    await note(t, user, loanId, `Prêt créé pour ${clientName} : ${principal} ${cur} à ${rate} % sur ${days} jours, à rembourser ${totalDue} ${cur} avant le ${dueDate}`);
...
const paidOf = async (t, id) => round2(Number((await t.get('SELECT COALESCE(SUM(amount), 0) AS s FROM payments WHERE loan_id = ?', [id])).s));
...
add('POST', '/api/loans/:id/payments', 'admin', async ({ db, cfg, user, params, body }) => {
  ...
  const pnote = str(body.note, 200), cur = cfg.settings.currency || '';
  return mutate(db, async t => {
    ...
    const remaining = round2(loan.total_due - await paidOf(t, loan.id));
    if (value > remaining + 0.004) throw new HttpError(400, `Le montant dépasse le reste dû (${remaining} ${cur})`);
    ...
    await note(t, user, loan.id, `Paiement de ${value} ${cur} reçu le ${date}${pnote ? ' (' + pnote + ')' : ''}`);
    return { ok: true };
  });
});
...
add('PUT', '/api/settings', 'admin', async ({ db, cfg, user, body }) => {
  ...
  const maxDays = Number(body.maxDays);
  const timezone = str(body.timezone, 60) || cfg.settings.timezone;
  if (!Number.isFinite(defaultRate) || defaultRate < 0 || defaultRate > 100) throw new HttpError(400, 'Taux par défaut invalide');
  if (!Number.isInteger(maxDays) || maxDays < 1 || maxDays > 365) throw new HttpError(400, 'Durée maximale invalide (1 à 365 jours)');
  if (!validTz(timezone)) throw new HttpError(400, 'Fuseau horaire invalide');
  return mutate(db, async t => { await setSettings(t, { company, currency, defaultRate, maxDays, timezone }); await logGlobal(t, user, 'Réglages modifiés'); return { ok: true }; });
});
...
add('POST', '/api/restore', 'admin', async ({ db, cfg, user, body }) => {
  ...
  for (const l of d.loans) {
    st.push({ sql: 'INSERT INTO loans(id, client_id, principal, rate, days, start_date, due_date, interest, total_due, guarantee, notes, written_off_date, created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',
      args: [str(l.id, 40), str(l.clientId, 40), n0(l.principal), n0(l.rate), Math.round(n0(l.days)), str(l.startDate, 10), str(l.dueDate, 10), n0(l.interest), n0(l.totalDue), str(l.guarantee, 200), str(l.notes, 1000), l.writtenOff && l.writtenOff.date ? str(l.writtenOff.date, 10) : null, str(l.createdAt, 40) || nowIso()] });
